...
public class TsTypeName(Type type)
{
    ...
    private static string GetName(Type type)
    {
        ...
        var rootTypeName = GetTypeNameBase(effectiveType);

        if (effectiveType.IsGenericType && Nullable.GetUnderlyingType(effectiveType) is null)
        {
            var genericArguments = type.GetGenericArguments();
            var genericArgumentsNames = genericArguments.Select(ga => GetName(ga));

            return $"{rootTypeName}<{string.Join(", ", genericArgumentsNames)}>";
        }

        return isEnumerable ? $"{rootTypeName}[]" : rootTypeName;
    }

    private static string GetTypeNameBase(Type type)
    {
        if (Nullable.GetUnderlyingType(type) is { } underlyingType)
        {
            return GetTypeNameBase(underlyingType);
        }
